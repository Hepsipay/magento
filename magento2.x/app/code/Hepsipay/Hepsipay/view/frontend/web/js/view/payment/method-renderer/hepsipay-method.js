...
define(
    [
        ...
        'Hepsipay_Hepsipay/js/model/validator',
        'mage/validation'           
    ],
    function ($, ko, quote, priceUtils, url, Component) {
        ...
        var url_cc = url.build('hepsipay/payment/index');
        var url_submit = url.build('hepsipay/payment/cardinfo');
        var url_bkm = url.build('hepsipay/payment/salebkm');
        var url_redirect = url.build('hepsipay/payment/redirectaction');
        var url_image = url.build('pub/static/frontend/Magento/luma/en_US/Hepsipay_Hepsipay/images/');
        ...
        return Component.extend({
            defaults: {
                template: 'Hepsipay_Hepsipay/payment/form',
                transactionResult: ''
            },
            initialize: function () {
                this._super();
            ...
            get3DSecure: function() {
                return window.checkoutConfig.payment.hepsipay.threed_secure ;
            },
            getForce3DSecure: function() {
                return window.checkoutConfig.payment.hepsipay.force_threed_secure ;
            },
            isEnable3DSecure: function() {
              if(this.get3DSecure()=='1' || this.getForce3DSecure()=='1') {
                return true;
              }
              return false;
            },
            getMinOrderTotal: function() {
                return window.checkoutConfig.payment.hepsipay.minimum_order;
            },
            ...
            isInstallmentActive: function(){
                 return window.checkoutConfig.payment.hepsipay.installment;
            },
            setInputImage: function(value)
            {   
                var first_n_char = value.substring(0, 1);
                if(first_n_char=='4'){
                    $("#input-cc-number").css('background','rgba(0, 0, 0, 0) url("'+url_image+'card_type_images/hepsipay_creditcard_visa.png") no-repeat scroll right center / 8% auto');
                }
                else if(first_n_char=='5'){
                    $("#input-cc-number").css('background','rgba(0, 0, 0, 0) url("'+url_image+'card_type_images/hepsipay_creditcard_master.png") no-repeat scroll right center / 8% auto');
                }
                else if(first_n_char=='6'){
                    $("#input-cc-number").css('background','rgba(0, 0, 0, 0) url("'+url_image+'card_type_images/hepsipay_creditcard_maestro.png") no-repeat scroll right center / 8% auto');
                }
                else if(first_n_char=='9'){
                    $("#input-cc-number").css('background','rgba(0, 0, 0, 0) url("'+url_image+'card_type_images/hepsipay_creditcard_troy.png") no-repeat scroll right center / 8% auto');
                }
                else{        
                    $("#input-cc-number").css('background','rgba(0, 0, 0, 0) url("'+url_image+'card_type_images/hepsipay_creditcard_not_supported.png") no-repeat scroll right center / 8% auto');
                }
                return true;
            },
                ...
                if(cc_bin_length == 6 || (callflag==1 && cc_bin_length ==16) || (prevcno == currcardno && cc_bin_length==5) || (prv_length>0 && cc_bin_length==0)){  
                    ...
                    $.ajax({
                        ...
                        {   
                            ...
                            if(result.data.type != 'CREDIT' || window.checkoutConfig.payment.hepsipay.force_threed_secure =='1'){
                                $('.hepsipay-checkbox').show();
                                document.getElementById("use3d").value = 1;
                                document.getElementById("use3d").checked = 1;
                                document.getElementById("use3d").disabled = 1;
                                result.has3d = 1;
                            }else{
                                document.getElementById("use3d").removeAttribute('disabled');
                            }
                                ...
                                if(grandtotal >= min_order){
                                    ...
                                    if(result.installments != null ){    
                                        ...
                                        for (var j = 0; j < installments_length; j++) { /*for start*/
                                            var commission = parseFloat(result.installments[j].commission);
                                            var installment_count = j + 1;

                                            if(window.checkoutConfig.payment.hepsipay.installment_commission != '1') commission = 0;
                                            if(installment_count < 2) commission = 0;
            ...
            submitForm: function(){
                if (this.validateForm('#hepsipay-form')) {
